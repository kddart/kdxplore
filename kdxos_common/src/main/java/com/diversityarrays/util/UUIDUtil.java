/*
 * Originally found in org.apache.logging.log4j.core.util.UuidUtil.
 */
...
public final class UUIDUtil {
     ...
     */
    public static final String UUID_SEQUENCE = "org.apache.logging.log4j.uuidSequence"; //$NON-NLS-1$
    private static final String ASSIGNED_SEQUENCES = "org.apache.logging.log4j.assignedSequences"; //$NON-NLS-1$
    ...
    static {
        ...
        try {
            ...
            try {
                ...
                if (ni != null && !ni.isLoopback() && ni.isUp()) {
                    Method method = ni.getClass().getMethod("getHardwareAddress"); //$NON-NLS-1$
                    if (method != null) {
        ...
        synchronized (runtime) {
            ...
            if (assigned == null) {
                ...
                String[] array = assigned.split(","); //$NON-NLS-1$
            ...
            assigned = assigned == null ? Long.toString(rand) : assigned + "," + Long.toString(rand); //$NON-NLS-1$
    ...
    public static String getUuidAsFourCharacterGroups() {
    	UUID uuid = getTimeBasedUUID();
    	String uuid_s = uuid.toString();
		String[] parts = uuid_s.split("-", 0); //$NON-NLS-1$
		...
		for (String p : parts) {
			if (p.length() <= 4) {
				...
				int spos = 0;
				while (spos < plen) {
					sb.append('-').append(p.substring(spos, spos + Math.min(4, plen - spos)));
					spos += 4;
				}
			}
		}
