...
public class ElapsedDaysTraitValidationProcessor extends NumericTraitValidationProcessor {
	
//	private final DateFormat dateFormat;
//	private Date trialPlantingDate;

	public ElapsedDaysTraitValidationProcessor(ValidationRule vr /*, Date pd */) {
		super(vr);
//		trialPlantingDate = pd;
//		dateFormat = TraitValue.getTraitValueDateFormat();
	}
	...
	public Either<TraitValueType,Number> isTraitValueValid(String traitValue) {
		...
		if (TraitValueType.SET.equals(tvt)) {
			try {
				...
				int days = Integer.parseInt(traitValue);
//				Date date = dateFormat.parse(traitValue);
//				int days = DateDiffChoice.differenceInDays(trialPlantingDate, date);
				if (validationRule==null || validationRule.evaluate(traitValue)) {
