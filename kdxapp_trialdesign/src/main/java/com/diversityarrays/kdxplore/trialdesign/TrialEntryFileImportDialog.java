...
public class TrialEntryFileImportDialog extends EntryFileImportDialog<TrialEntryFile,TrialHeading>{
    ...
    public TrialEntryFileImportDialog(Window owner, String title, File inputFile) {
        super(owner, title, inputFile, (role) -> TrialHeading.ENTRY_TYPE==role);
    }
            ...
            Map<String, TrialHeading> roleByHeading) throws IOException, EntryFileException
    {
        ...
        String normalEntryName = getNormalEntryName();
        if (isSaveForFutureChecked()) {
        	TrialDesignPreferences.getInstance().setNormalEntryTypeName(normalEntryName);
        }
        return new TrialEntryFile(entryParams, normalEntryName);
    }
